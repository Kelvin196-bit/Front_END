...
export default function CartCard({
  ...
  return (
    <div className={`${className} lg:flex flex-col`}>
      {color && size && price && <hr className="my-5 bg-light-gray-2" />}
      <div className="flex flex-col lg:flex-row justify-between">
        <div className="flex">
          <img
            className="h-[104px] w-[127px] mr-5 box-border object-contain bg-light-blue"
            src={image}
            alt={title}
          />
          <span>
            ...
            {color && (
              <p className="py-2 text-sm font-medium text-light-gray">
                Cor: <span className="text-dark-gray">{color}</span>
              </p>
            )}
            {size && (
        ...
        {showCounter && (
          <div className="flex flex-col lg:flex-row lg:items-center gap-7 lg:gap-9">
            <div className="flex flex-col items-center lg:pr-2 pt-5 lg:pt-0">
              <h1 className="block lg:hidden font-medium font-sm self-start pb-3 text-dark-gray-2">
                QUANTIDADE
              </h1>
              <div className="flex items-center pb-2">
                <button
                  className="h-9 w-[85px] lg:w-9 border border-light-gray-2 rounded-sm"
              ...
              <button
                className="text-md hover:font-bold text-red-600 underline"
                onClick={decreaseItem}
              >
                Remover item
              </button>
      ...
      {!showCounter && price && (
        <div>
          ...
          <p className="flex justify-between font-bold text-base text-dark-gray">
            Valor total: <span className="text-error">R$ {total}</span>
          </p>
        </div>
      )}
    </div>
